...
void Player::Update() {
	m_transform.SetAngle(m_transform.GetAngle() + (100 * GetDeltaTime()));
	...
	if (m_inputManager.GetKey(InputManager::Key::Left))
		m_transform.SetPos(m_transform.GetPos().x - (300.0f * GetDeltaTime()), m_transform.GetPos().y);

	if (m_inputManager.GetKey(InputManager::Key::Right))
		m_transform.SetPos(m_transform.GetPos().x + (300.0f * GetDeltaTime()), m_transform.GetPos().y);

	if (m_inputManager.GetKey(InputManager::Key::Up))
		m_transform.SetPos(m_transform.GetPos().x, m_transform.GetPos().y - (300.0f * GetDeltaTime()));
